...
HUD::HUD() {
    ...
    if (_panicBarTexture.id == 0 || _panicBarIcon.id == 0 || _bonusTexture.id == 0 || _timeTexture.id == 0 || _panel.id == 0) {
...
void HUD::draw() {
    ...
    DrawTextureEx(_panicBarIcon, {static_cast<float>(panicIconX), static_cast<float>(hudY)}, 0.0f, 1.0f, WHITE);
    
    int frameWidth = _panicBarTexture.width / 16;
    Rectangle source = { static_cast<float>(_panicCount * frameWidth), 0, static_cast<float>(frameWidth), static_cast<float>(_panicBarTexture.height) };
